...
public class PostRequestPOJO {
	...
	void createUser() {
		//serialization
		PostUser user= new PostUser("Sam","Manager");
		Response resp= given().header(API_Info.HEADER_NAME,API_Info.API_KEY).contentType(ContentType.JSON).
				body(user).when().post("/users").then().statusCode(201).body("name",equalTo("Sam")).extract().response();
		
		//deserialzation
		PostUser cnt=  resp.as(PostUser.class);
